<script>
    ...
    const loadInvoices = () => {
      const savedInvoiceIds = JSON.parse(localStorage.getItem("ig.ids")) || [];
      const invoices = [];
      for (let i = 0; i < savedInvoiceIds.length; i++) {
        const invoice = JSON.parse(localStorage.getItem("ig.invoice." + savedInvoiceIds[i]));
        if (invoice) {
          invoices[savedInvoiceIds[i]] = invoice;
        }
      }
    ...
    const searchItems = () => {
      ...
      for (let i = 0; i < savedInvoiceIds.length; i++) {
        const invoice = JSON.parse(localStorage.getItem("ig.invoice." + savedInvoiceIds[i]));
        if (invoice) {
          if (!search || (invoice.invoiceTo && invoice.invoiceTo.toLowerCase().includes(search.toLowerCase()))) {
            invoices[savedInvoiceIds[i]] = invoice;
          }
        }
      }
    ...
    const removeInvoice = (id) => {
      const savedInvoiceIds = JSON.parse(localStorage.getItem("ig.ids")) || [];
      localStorage.removeItem("ig.invoice." + id);
      const newIds = savedInvoiceIds.filter(item => item !== id);
      localStorage.setItem("ig.ids", JSON.stringify(newIds));
      loadInvoices();
    };
  ...
  <section class="container myinvoicePart">
    <div class="columns is-mobile myinvoicePart">
      <div class="column is-offset-2-desktop is-8-desktop is-offset-2-tablet is-8-tablet is-offset-1-mobile is-10-mobile myinvoicePart-Color">
        ...
        <nav class="field navPart">
          ...
          <div class="control newInvoice-Button">
            <a class="button is-success" on:click={() => goto('/')}>
              New Invoice
            </a>
          </div>
        </nav>
                    ...
                    <a href={`/#/${key}`}>
                      <div class="boxDesign">
                        <figure class="image is-64x64 boxImage">
                          <img src="https://eosillumination.com/wp-content/uploads/2017/11/Pdf-image-281x300.png" alt="PDF Icon" />
                        </figure>
        ...
        <p class="theseInvoice">
          These invoices are stored in your device and not online. Clearing your
          browser cache could cause you to lose these invoices. We recommend keeping a copy of each invoice.
        </p>
  
        <div class="clearEverything">
          <button class="button is-link" on:click={clearData}>
            Clear Everything
          </button>
        </div>
      </div>

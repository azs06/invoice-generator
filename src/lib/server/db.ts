...
export async function getInvoice(db: D1Database, id: string, userId?: string): Promise<InvoiceData | null> {
    const d1 = drizzle(db);
    ...
    const result = await d1.select().from(invoices).where(and(...conditions)).get();

    if (!result) return null;
    return JSON.parse(result.data) as InvoiceData;
}

export async function saveInvoice(db: D1Database, bucket: R2Bucket | undefined, id: string, invoiceData: InvoiceData, userId: string): Promise<void> {
    const d1 = drizzle(db);
    const data = JSON.stringify(invoiceData);
    const now = new Date();

    // Check if invoice exists
    const existing = await d1.select({ id: invoices.id }).from(invoices).where(eq(invoices.id, id)).get();

    if (!existing) {
        // Check limit
        const countResult = await d1.select({ count: count() }).from(invoices).where(eq(invoices.userId, userId)).get();
        const currentCount = countResult?.count ?? 0;

        if (currentCount >= INVOICE_LIMIT) {
            // Find oldest invoice
            const oldest = await d1.select().from(invoices)
...
export async function deleteInvoice(db: D1Database, bucket: R2Bucket | undefined, id: string, userId: string): Promise<void> {
    const d1 = drizzle(db);

    // Get invoice to check for PDF key
    const invoice = await d1.select().from(invoices).where(and(eq(invoices.id, id), eq(invoices.userId, userId))).get();
...
export async function getAllInvoices(db: D1Database, userId: string): Promise<SavedInvoiceRecord[]> {
    const d1 = drizzle(db);
    const results = await d1.select().from(invoices)
        .where(eq(invoices.userId, userId))
...
export async function clearAllInvoices(db: D1Database, bucket: R2Bucket | undefined, userId: string): Promise<void> {
    const d1 = drizzle(db);
...
export async function getInvoiceCount(db: D1Database, userId: string): Promise<number> {
    const d1 = drizzle(db);
    const result = await d1.select({ count: count() }).from(invoices).where(eq(invoices.userId, userId)).get();
    return result?.count ?? 0;
}

export async function updateInvoicePdfKey(db: D1Database, id: string, userId: string, pdfKey: string): Promise<void> {
    const d1 = drizzle(db);
    await d1.update(invoices)
        .set({ pdfKey })
...
export async function createShareLink(
    ...
    const invoice = await d1.select().from(invoices)
...
export async function getShareLinks(
    ...
    const results = await d1.select().from(sharedLinks)
...
export async function getInvoiceByShareToken(
    ...
    const link = await d1.select().from(sharedLinks)
